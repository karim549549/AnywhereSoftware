"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Quiz = void 0;
const swagger_1 = require("@nestjs/swagger");
const question_entity_1 = require("./question.entity");
class Quiz {
    id;
    title;
    description;
    questions;
    createdAt;
    updatedAt;
}
exports.Quiz = Quiz;
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Unique identifier of the quiz' }),
    __metadata("design:type", String)
], Quiz.prototype, "id", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Title of the quiz' }),
    __metadata("design:type", String)
], Quiz.prototype, "title", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Description of the quiz', required: false }),
    __metadata("design:type", Object)
], Quiz.prototype, "description", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ type: [question_entity_1.Question], description: 'List of questions in the quiz' }),
    __metadata("design:type", Array)
], Quiz.prototype, "questions", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Date and time when the quiz was created' }),
    __metadata("design:type", Date)
], Quiz.prototype, "createdAt", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Date and time when the quiz was last updated' }),
    __metadata("design:type", Date)
], Quiz.prototype, "updatedAt", void 0);
//# sourceMappingURL=quiz.entity.js.map